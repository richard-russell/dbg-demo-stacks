deployment "lz_20_dev" {
  inputs = {
    name                = "stack-lz-20-dev"
    cidr                = "10.20.0.0/16"
    environment         = "dev"
    region              = "eu-west-1"
    enable_s3           = true
    enable_ssm_advanced = true
    extra_tags          = { Demo = "demo-3-stacks", Deployment = "lz-20-dev", Environment = "dev" }
...
deployment "lz_20_test" {
  inputs = {
    name                = "stack-lz-20-test"
    cidr                = "10.20.0.0/16"
    environment         = "test"
    region              = "eu-west-1"
    enable_s3           = true
    enable_ssm_advanced = true
    extra_tags          = { Demo = "demo-3-stacks", Deployment = "lz-20-test", Environment = "test" }
    role_arn            = local.role_arn
    identity_token      = identity_token.aws.jwt
  }
